...
from data.external.dataset_data_format import CENTER_X_COORD, CENTER_Z_COORD, CENTER_Y_COORD, SCENARIO_DIR_PATTERN, \
    LABEL_FILES_PATTERN, TRACKER_ID, FRAME_NUMBER_OF_DIGITS, \
    GPS_FILES_PATTERN, VELOCITY_X, VELOCITY_Y, VELOCITY_Z, REFERENCE_POINT_3D, APPROX_FRAME_DURATION, LABEL
from src.libraries.myFileSystemLib import getNextFile, log_alert
...
def main(plot=False):
    ...
    filenames = os.listdir(PATH_DIR_INTERIM_DATA + scenario)  # list files in scenario directory
    filenames.sort()
    for filename in filenames:
        path_at_i = PATH_DIR_INTERIM_DATA + scenario + '/' + filename
        # frame_i id
        ...
        if fnmatch.fnmatch(filename, LABEL_FILES_PATTERN):
            ...
            path_frame_at_i = PATH_DIR_INTERIM_DATA + scenario + '/' + filename
            # get position at frame_i
            objects_at_i = pd.read_csv(path_frame_at_i,
                                       usecols=[LABEL, TRACKER_ID, CENTER_X_COORD, CENTER_Y_COORD, CENTER_Z_COORD])
            ...
            for object_index in range(0, number_of_objects):
                ...
                if a0[0] < 0:  # the considered traffic participant is behind the instrumented car --> do not
                ...
                if not os.path.exists(path_frame_at_ii):
                    continue
                objects_at_ii = pd.read_csv(path_frame_at_ii, usecols=[TRACKER_ID, CENTER_X_COORD, CENTER_Y_COORD,
                                                                       CENTER_Z_COORD])
                ...
                if shortest_distance <= THRESHOLD_DISTANCE:
                    ...
                    log_alert('trajectories prediction v1', scenario, frame_i_id, objects_at_i.loc[object_index, LABEL],
